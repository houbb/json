 ...
 */
@ThreadSafe
public class ArrayBooleanSerialize implements ISerialize<boolean[]> {

    @Override
    public String serialize(boolean[] objects, ISerializeContext context) {
        if(ArrayPrimitiveUtil.isEmpty(objects)) {
            return JsonIterableConst.EMPTY;
        }

        List<String> stringList = ArrayPrimitiveUtil.toList(objects, new IHandler<Boolean, String>() {
            @Override
            public String handle(Boolean object) {
